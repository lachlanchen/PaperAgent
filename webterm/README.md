...
- The control panel can generate an SSH key, write GitHub SSH config (for both `/home/<user>` and `/root`), and show the public key in the UI.
...
- Dev reload is opt-in: add `?dev=1` to the URL or set `localStorage.paperagent.dev = "1"`.
...
- The Codex panel includes buttons to create a project `.gitignore` and run status/add/commit/push.
